...
# Promotion window indicator (March 13-27, 2026; off-peak = 2x usage)
# Peak: 8 AM–2 PM ET (EDT = UTC-4 → 12:00–18:00 UTC)
promo_status=""
today_date=$(date +%Y%m%d)
if [[ "$today_date" -ge 20260313 && "$today_date" -le 20260327 ]]; then
    et_hour=$(TZ=America/New_York date +%H)
    et_min=$(TZ=America/New_York date +%M)
    if [[ "$et_hour" -ge 8 && "$et_hour" -lt 14 ]]; then
        mins_until=$(( (14 - et_hour) * 60 - 10#$et_min ))
        ...
        promo_status=" | ${C_ACCENT}⏳ 2x in ${countdown}${C_GRAY}"
    else
        # Off-peak: count down to next 8 AM ET (start of peak)
        if [[ "$et_hour" -lt 8 ]]; then
            mins_to_peak=$(( (8 - et_hour) * 60 - 10#$et_min ))
        else
            mins_to_peak=$(( (32 - et_hour) * 60 - 10#$et_min ))
        fi
        if [[ $mins_to_peak -ge 60 ]]; then
            hrs=$(( mins_to_peak / 60 ))
            mins=$(( mins_to_peak % 60 ))
            time_left="${hrs}h ${mins}m left"
        else
            time_left="${mins_to_peak}m left"
        fi
        promo_status=" | \033[38;5;71m🚀 2x ON | ${time_left}${C_GRAY}"
    fi
fi
